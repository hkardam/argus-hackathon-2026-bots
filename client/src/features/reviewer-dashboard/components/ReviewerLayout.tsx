...
import { LayoutDashboard, ClipboardCheck, LogOut, Bell, Leaf, Menu } from 'lucide-react';
...
export default function ReviewerLayout() {
  ...
  const navItems = [
    ...
    { name: 'My Reviews', path: '/reviewer/reviews', icon: ClipboardCheck },
  ];

  const initials = user?.name?.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2) ?? 'RV';
    ...
    <div className="min-h-screen bg-slate-50 flex font-sans">
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-slate-200 transform transition-transform duration-200 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 lg:static lg:block flex flex-col`}>
        ...
        <div className="px-4 py-3 border-b border-slate-100">
          <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Reviewer</span>
        </div>
        <nav className="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
          {navItems.map(item => (
            <NavLink key={item.name} to={item.path} end={item.path === '/reviewer/dashboard'} onClick={() => setIsSidebarOpen(false)}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 sm:px-6 lg:px-8 sticky top-0 z-40">
          ...
          <div className="flex items-center gap-4 ml-auto">
            ...
            <div className="h-8 w-8 bg-violet-100 rounded-full flex items-center justify-center text-violet-700 font-bold text-sm border border-violet-200">{initials}</div>
          </div>
        </header>
